...
import { CircleCheck, Building2, Users } from 'lucide-react'
import { Section, Reveal, GlowCard } from '../components/ui/Primitives.jsx'
...
export default function Demo() {
  ...
  function handleSubmit(e) {
    e.preventDefault()
    // Placeholder — wire to a form service or serverless function later.
    // Vite exposes VITE_-prefixed env vars: import.meta.env.VITE_CONTACT_ENDPOINT
    setSent(true)
  }
  ...
  return (
    <section className="relative overflow-hidden pt-28 sm:pt-36 pb-20">
      ...
      <div className="pointer-events-none absolute -top-40 left-1/2 h-[380px] w-[640px] -translate-x-1/2 rounded-full bg-electric/12 blur-[120px]" aria-hidden />
        ...
        <div className="grid gap-12 lg:grid-cols-2 lg:items-start">
          <Reveal>
            ...
            <ul className="mt-8 space-y-3">
              {['30-minute walkthrough with the founding team', 'Live trust engine demo on a sample workflow', 'Pilot scoping for design partners'].map((t) => (
                <li key={t} className="flex items-start gap-2.5 text-sm text-slatemute">
                  <CircleCheck className="mt-0.5 h-4 w-4 shrink-0 text-verify" aria-hidden /> {t}
                </li>
              ))}
            </ul>
          </Reveal>
            ...
            <GlowCard className="p-6 sm:p-8">
              {sent ? (
                <div className="py-10 text-center">
                  ...
                  <h2 className="mt-4 font-display text-xl font-semibold text-white">Request received</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                      ...
                      return (
                        <button
                          ...
                          className={`rounded-xl border p-3.5 text-left transition ${
                            active ? 'border-electric/50 bg-electric/10' : 'border-hairline bg-panel2/40 hover:border-electric/30'
                          }`}
                        >
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div>
                      <label htmlFor="name" className="telemetry mb-1.5 block">Name</label>
                      <input id="name" name="name" required autoComplete="name" className={inputCls} placeholder="Jordan Lee" />
                    </div>
                    <div>
                      <label htmlFor="email" className="telemetry mb-1.5 block">Work email</label>
                      <input id="email" name="email" type="email" required autoComplete="email" className={inputCls} placeholder="jordan@company.com" />
                    </div>
                  </div>
                  <div>
                    <label htmlFor="company" className="telemetry mb-1.5 block">Company</label>
                    <input id="company" name="company" autoComplete="organization" className={inputCls} placeholder="Company or organization" />
                  </div>
                  <div>
                    <label htmlFor="message" className="telemetry mb-1.5 block">Where do your agents need a trust layer?</label>
                    <textarea id="message" name="message" rows="4" className={inputCls} placeholder="e.g. Our support agents issue refunds and we need approval flows above $500…" />
                  </div>
                  <button
                    type="submit"
